...
local ITEMBUTTON_SIZE = 39

PLGuildBankClassic.Item = {}
PLGuildBankClassic.Item.prototype = Item
PLGuildBankClassic.Item.count = 0
PLGuildBankClassic.Item.pool = nil
function PLGuildBankClassic.Item:Create()
	...
	if item then
		...
		item = CreateFrame("Button", ("PLGuildBankClassicItemButton%d"):format(self.count), nil, "ContainerFrameItemButtonTemplate")
...
function PLGuildBankClassic.Item:WrapItemButton(item)
	...
	item.searchOverlay:SetSize(ITEMBUTTON_SIZE, ITEMBUTTON_SIZE)
	item.searchOverlay:SetPoint("CENTER")

	return item
end

function PLGuildBankClassic.Item:CreateItemPool()
	self.pool = {}
	for c = 1, NUM_CONTAINER_FRAMES do
		for i = 1, MAX_CONTAINER_ITEMS do
			local item = _G[("PLGBCContainerFrame%dItem%d"):format(c, i)]
			if item then
				item:SetID(0)
				item:ClearAllPoints()
...
function Item:Free()
	...
	PLGuildBankClassic.Item.pool[self] = true
end
